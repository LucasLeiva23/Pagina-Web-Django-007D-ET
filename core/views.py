...
def registrarme(request):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            mensaje = f'Tu cuenta de usuario: "{usuario.username}" ha sido creado con exito'
    ...
    if request.method == 'POST':
        ...
        if form:
            if form.is_valid():
                ...
                messages.success(request, f'El produco "{str(producto)}" se logro {accion} correctamente')
...
def bodega(request):
    ...
    if request.method == 'POST':
        ...
        if cantidad == 1:
            ...
            messages.success(request, f'Se ha agregado {cantidad} producto de "{producto.nombre}" a la bodega')
...
def eliminar_producto_en_bodega(request, bodega_id):
    ...
    nombre_producto = bodega.producto.nombre
...
def agregar_producto_al_carrito(request, producto_id):
    if not request.user.is_authenticated:
        messages.info(request, 'Para poder comprar, primero debes iniciar sesión.')
        return redirect('inicio')

    perfil = request.user.perfil
    producto = Producto.objects.get(id=producto_id)

    # Verificar el tipo de usuario para compras
    if not es_cliente_activo(perfil):
        messages.error(request, f'Para poder comprar debes tener cuenta de Cliente, pero tu cuenta es de {perfil.tipo_usuario}.')
        return redirect('inicio')

    # Calcular precios y descuentos del producto
    precio_normal, precio_oferta, precio_subscr, hay_desc_oferta, hay_desc_subscr = calcular_precios_producto(producto)
    ...
    # Redireccionar a la página de detalle del producto añadido
